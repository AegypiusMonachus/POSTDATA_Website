...
from mee_nu.mee_nu_util import MeeNu
...
if __name__ == "__main__":
    ...
    sql = "select * from mee_nu order by id DESC limit 1"
    ...
    with open(g_var.ENV_DIR+'/mee_nu/config.json', 'w') as f:
    ...
    for i in range(0, ADD_ONE_ASSIGNMENT_THREAD_NUM):
        obj_list.append(MeeNu(EACH_THREAD_ASSIGNMENT_NUM + 1))
    if EACH_THREAD_ASSIGNMENT_NUM != 0:
        for i in range(0, REMAIN_THREAD_NUM):
            obj_list.append(MeeNu(EACH_THREAD_ASSIGNMENT_NUM))
