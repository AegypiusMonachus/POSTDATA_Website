...
from genius_com.genius_com_util import GeniusCom
from project_utils.project_util import get_command_line_arguments, get_global_params, send_spider_status, send_spider_block_status, MysqlHandler
from project_utils import g_var


if __name__ == "__main__":
    present_website = "genius_com"
    VPN = "en"
    ...
    for i in range(0, ADD_ONE_ASSIGNMENT_THREAD_NUM):
        obj_list.append(GeniusCom(EACH_THREAD_ASSIGNMENT_NUM+1))
    if EACH_THREAD_ASSIGNMENT_NUM != 0:
        for i in range(0, REMAIN_THREAD_NUM):
            obj_list.append(GeniusCom(EACH_THREAD_ASSIGNMENT_NUM))
