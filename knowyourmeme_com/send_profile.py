...
import threading
import sys
sys.path.append('./')                          # 在window下的SendArticle目录执行
sys.path.append('/home/web_python/project/SendArticle')    # 在linux服务器下执行
...
if __name__ == "__main__":
    ...
    for i in range(0, ADD_ONE_ASSIGNMENT_THREAD_NUM):
        obj_list.append(KnowyourmemeCom(EACH_THREAD_ASSIGNMENT_NUM + 1))
    if EACH_THREAD_ASSIGNMENT_NUM != 0:
        for i in range(0, REMAIN_THREAD_NUM):
            obj_list.append(KnowyourmemeCom(EACH_THREAD_ASSIGNMENT_NUM))
